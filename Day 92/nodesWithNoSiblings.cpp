...
void solve(Node *node, vector<int> &noSiblings)
{
    if (!node)
        return;
    ...
    solve(node->left, noSiblings);
    solve(node->right, noSiblings);
}

vector<int> noSibling(Node *node)
{
    vector<int> noSiblings;
    solve(node, noSiblings);

    if (noSiblings.size() == 0)
        noSiblings.push_back(-1);
